document.addEventListener("DOMContentLoaded", function () {
      ...
      .then(function (geojson) {
        ...
        if (trackFeature && trackFeature.geometry.coordinates.length > 0) {
          ...
          el.innerHTML =
            ...
            "</svg>";
          currentPosMarker = new maplibregl.Marker({
            element: el,
            anchor: "bottom",
          })
            .setLngLat(lastCoord)
  ...
  var allEntries = Array.from(
    document.querySelectorAll(".timeline-entry[data-lat]"),
  );
  ...
  document
    .querySelectorAll(".timeline-dot, .timeline-date")
    .forEach(function (el) {
  ...
  function positionDistanceLabels() {
    ...
    for (var i = 0; i < entries.length; i++) {
      ...
      var midY =
        (prevRect.top + prevRect.bottom + curRect.top + curRect.bottom) / 4;
  ...
  function showMedia() {
    ...
    if (isVideo) {
      lightboxMedia.innerHTML =
        '<video class="lightbox-img" src="' +
        el.src +
        '" controls autoplay></video>';
    } else {
      var fullSrc = el.dataset.full || el.src;
      lightboxMedia.innerHTML =
        '<img class="lightbox-img" src="' + fullSrc + '">';
    }
  ...
  document
    .querySelectorAll(".timeline-photos .video-wrap")
  ...
  document.querySelectorAll(".timeline-photos-wrap").forEach(function (wrap) {
    ...
    function updateArrows() {
      ...
      var canScrollRight =
        strip.scrollLeft < strip.scrollWidth - strip.clientWidth - 5;
  ...
  function highlightEntry(el) {
    document.querySelectorAll(".timeline-entry.active").forEach(function (e) {
  ...
  if (typeof isMobile !== "undefined" && isMobile) {
    ...
    mobileCards.forEach(function (card, idx) {
      ...
      var photos = entryEl.querySelectorAll(
    ...
    function startAutoAdvance() {
      ...
      var currentBar = viewerProgress.querySelectorAll(".viewer-progress-bar")[
        currentPageIdx
      ];
      if (currentBar) {
    ...
    function renderViewer() {
      ...
      topDate.textContent =
        entry.meta.date + (entry.meta.time ? " · " + entry.meta.time : "");
      topKm.textContent =
        entry.meta.totalKm > 0 ? entry.meta.totalKm + " km" : "";
      ...
      entry.pages.forEach(function (_, i) {
        ...
        if (i < currentPageIdx) {
          ...
          fill.style.animation =
            "progress-fill " + duration / 1000 + "s linear forwards";
        }
    ...
    function highlightCard(idx) {
      mobileCards.forEach(function (c) {
        c.classList.remove("mobile-card-active");
      });
      if (mobileCards[idx])
        mobileCards[idx].classList.add("mobile-card-active");
    }
    ...
    function updateProgressBar() {
      ...
      if (
        entryPages[closest] &&
        entryPages[closest].meta.totalKm !== undefined
      ) {
    ...
    if (typeof isOwner !== "undefined" && isOwner) {
      ...
      mobileCards[initialIdx].scrollIntoView({
        inline: "center",
        block: "nearest",
      });
    }
